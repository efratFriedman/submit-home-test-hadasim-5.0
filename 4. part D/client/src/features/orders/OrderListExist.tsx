...
import { fetchOrdersNotCompleted, UpdateStatusOrder } from './orders';
import './orderListGrocer.css';

const OrderListExist = () => {
    const orders = useSelector((state: RootState) => state.order.ordersNotCompleted);
    const dispatch = useDispatch<AppDispatch>();
    const [flippedOrders, setFlippedOrders] = useState<{ [key: number]: boolean }>({});

    useEffect(() => {
        dispatch(fetchOrdersNotCompleted());
    }, [dispatch]);
